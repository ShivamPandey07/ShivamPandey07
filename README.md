<h1 align="center">Hi 👋, I'm Shivam Pandey</h1>

<h3 align="center">Senior Software Test Engineer | Automation Architect | Python Developer</h3>

<p align="center">
  🚀 Playwright • Cypress • Selenium • API Testing • Capital Markets
</p>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&amp;size=22&amp;duration=3000&amp;pause=1000&amp;center=true&amp;vCenter=true&amp;width=900&amp;lines=Playwright+Automation+Expert;Cypress+Framework+Developer;Senior+SDET;Capital+Markets+Specialist;Python+Developer;Creator+of+AI_Scalping" alt="Typing animation introducing Shivam Pandey's areas of expertise" />
</p>

---

## 👨‍💻 About Me

- 🔭 Senior Software Test Engineer with **6+ years of experience**
- 🚀 Specialist in **Playwright, Cypress, Selenium, and API testing**
- 📈 Experienced in **capital markets and equity trading systems**
- 💹 Hands-on experience with **FIX Protocol, OMS, and EMS platforms**
- 🐍 Python developer focused on test automation and quantitative research
- 🤖 Creator of **AI_Scalping**, a quantitative research and backtesting platform
- ☁️ Experienced with **Azure DevOps, Jenkins, Docker, and CI/CD**
- 📊 Strong background in data-driven strategy validation and trading technology

---

## 🛠️ Technology Ecosystem

### 🧪 Automation and Testing

<p align="center">
  <a href="https://playwright.dev/" target="_blank" rel="noreferrer"><img src="https://cdn.simpleicons.org/playwright" alt="Playwright" title="Playwright" width="48" height="48" /></a>&nbsp;&nbsp;
  <a href="https://www.cypress.io/" target="_blank" rel="noreferrer"><img src="https://cdn.simpleicons.org/cypress" alt="Cypress" title="Cypress" width="48" height="48" /></a>&nbsp;&nbsp;
  <a href="https://www.selenium.dev/" target="_blank" rel="noreferrer"><img src="https://cdn.simpleicons.org/selenium" alt="Selenium" title="Selenium" width="48" height="48" /></a>&nbsp;&nbsp;
  <a href="https://pytest.org/" target="_blank" rel="noreferrer"><img src="https://cdn.simpleicons.org/pytest" alt="Pytest" title="Pytest" width="48" height="48" /></a>&nbsp;&nbsp;
  <a href="https://cucumber.io/" target="_blank" rel="noreferrer"><img src="https://cdn.simpleicons.org/cucumber" alt="Cucumber" title="Cucumber" width="48" height="48" /></a>&nbsp;&nbsp;
  <a href="https://www.postman.com/" target="_blank" rel="noreferrer"><img src="https://cdn.simpleicons.org/postman" alt="Postman" title="Postman" width="48" height="48" /></a>
</p>

### 💻 Programming and Databases

<p align="center">
  <a href="https://www.python.org/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/python-colored.svg" alt="Python" title="Python" width="48" height="48" /></a>&nbsp;&nbsp;
  <a href="https://www.java.com/" target="_blank" rel="noreferrer"><img src="https://cdn.simpleicons.org/openjdk" alt="Java" title="Java" width="48" height="48" /></a>&nbsp;&nbsp;
  <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/javascript-colored.svg" alt="JavaScript" title="JavaScript" width="48" height="48" /></a>&nbsp;&nbsp;
  <a href="https://www.typescriptlang.org/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/typescript-colored.svg" alt="TypeScript" title="TypeScript" width="48" height="48" /></a>&nbsp;&nbsp;
  <a href="https://www.mysql.com/" target="_blank" rel="noreferrer"><img src="https://cdn.simpleicons.org/mysql" alt="MySQL" title="MySQL" width="48" height="48" /></a>&nbsp;&nbsp;
  <a href="https://www.postgresql.org/" target="_blank" rel="noreferrer"><img src="https://cdn.simpleicons.org/postgresql" alt="PostgreSQL" title="PostgreSQL" width="48" height="48" /></a>
</p>

### ⚙️ DevOps, Cloud, and CI/CD

<p align="center">
  <a href="https://git-scm.com/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/git-colored.svg" alt="Git" title="Git" width="48" height="48" /></a>&nbsp;&nbsp;
  <a href="https://github.com/ShivamPandey07" target="_blank" rel="noreferrer"><img src="https://www.readmecodegen.com/api/social-icon?name=github&amp;size=96" alt="GitHub" title="GitHub" width="48" height="48" /></a>&nbsp;&nbsp;
  <a href="https://azure.microsoft.com/" target="_blank" rel="noreferrer"><img src="https://cdn.simpleicons.org/microsoftazure" alt="Microsoft Azure" title="Microsoft Azure" width="48" height="48" /></a>&nbsp;&nbsp;
  <a href="https://www.jenkins.io/" target="_blank" rel="noreferrer"><img src="https://cdn.simpleicons.org/jenkins" alt="Jenkins" title="Jenkins" width="48" height="48" /></a>&nbsp;&nbsp;
  <a href="https://www.docker.com/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/docker-colored.svg" alt="Docker" title="Docker" width="48" height="48" /></a>&nbsp;&nbsp;
  <a href="https://www.linux.org/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/linux-colored.svg" alt="Linux" title="Linux" width="48" height="48" /></a>
</p>

---

## 🌎 Domain Expertise

| 📈 Capital Markets | ✈️ Aviation | 🚚 Supply Chain | 🛡️ Insurance |
|:---:|:---:|:---:|:---:|
| Equity Trading Systems | Airline Platforms | Logistics Systems | Policy Platforms |
| FIX Protocol | Booking Systems | Inventory Tracking | Claims Processing |
| OMS and EMS | Cargo Operations | Warehouse Automation | Underwriting |
| Market Data Testing | Flight Operations | Supply Chain Analytics | Risk Assessment |

---

## 🎯 Professional Snapshot

```yaml
Experience:
  - 6+ Years

Role:
  - Senior Software Test Engineer

Core Expertise:
  - UI Automation
  - API Automation
  - Framework Development
  - Python Engineering

Industries:
  - Capital Markets
  - Aviation
  - Supply Chain
  - Insurance

Current Focus:
  - Playwright Automation
  - AI-Assisted Testing
  - Trading Technology
  - Quantitative Research
```

---

## 💻 Technical Skills

<table>
  <tr>
    <td width="50%" valign="top">
      <h3>🧪 Test Automation</h3>
      <ul>
        <li><strong>UI automation:</strong> Playwright, Cypress, Selenium</li>
        <li><strong>Framework design:</strong> POM, Hybrid, Data Driven</li>
        <li><strong>BDD:</strong> Cucumber</li>
        <li><strong>Unit testing:</strong> Pytest</li>
        <li><strong>Execution:</strong> Parallel and distributed</li>
        <li><strong>Reporting:</strong> Allure and HTML reports</li>
      </ul>
    </td>
    <td width="50%" valign="top">
      <h3>🌐 API Testing</h3>
      <ul>
        <li><strong>REST API:</strong> Postman</li>
        <li><strong>Automation:</strong> REST Assured</li>
        <li><strong>Frameworks:</strong> Playwright API</li>
        <li><strong>Validation:</strong> Schema validation</li>
        <li><strong>Services:</strong> SOAP UI</li>
        <li><strong>Coverage:</strong> Contract testing</li>
      </ul>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3>💻 Programming</h3>
      <ul>
        <li>Python</li>
        <li>Java</li>
        <li>JavaScript</li>
        <li>TypeScript</li>
        <li>SQL</li>
      </ul>
    </td>
    <td width="50%" valign="top">
      <h3>⚙️ DevOps and CI/CD</h3>
      <ul>
        <li><strong>SCM:</strong> Git and GitHub</li>
        <li><strong>CI/CD:</strong> Jenkins and Azure DevOps</li>
        <li><strong>Containers:</strong> Docker</li>
        <li><strong>Platforms:</strong> Linux and Unix</li>
        <li><strong>Pipelines:</strong> Automated test execution</li>
        <li><strong>Monitoring:</strong> Logs and reports</li>
      </ul>
    </td>
  </tr>
</table>

---

## 📈 Capital Markets and 🚀 AI_Scalping

<table>
  <tr>
    <td width="48%" valign="top">
      <h3>📈 Capital Markets Expertise</h3>
      <ul>
        <li><strong>Trading systems:</strong> Equity trading platforms</li>
        <li><strong>Connectivity:</strong> FIX Protocol</li>
        <li><strong>OMS:</strong> Order Management Systems</li>
        <li><strong>EMS:</strong> Execution Management Systems</li>
        <li><strong>Trade flow:</strong> Front-office lifecycle</li>
        <li><strong>Market data:</strong> Real-time feed validation</li>
        <li><strong>Domain:</strong> Investment banking technology</li>
        <li><strong>Research:</strong> Algorithmic trading</li>
      </ul>
      <h4>Core Knowledge</h4>
      <p>
        FIX Protocol<br />
        OMS and EMS<br />
        Trade Lifecycle<br />
        Market Data Testing<br />
        Order Routing<br />
        Execution Flow<br />
        Front-Office Systems
      </p>
    </td>
    <td width="52%" valign="top">
      <h3>🚀 AI_Scalping</h3>
      <p>Python-powered quantitative research and backtesting platform for intraday trading strategies.</p>
      <h4>Key Features</h4>
      <ul>
        <li>✅ Historical backtesting</li>
        <li>✅ Strategy validation</li>
        <li>✅ Risk management engine</li>
        <li>✅ Paper trading simulation</li>
        <li>✅ Performance analytics</li>
        <li>✅ Multi-stock testing</li>
        <li>✅ Capital markets research</li>
      </ul>
      <h4>Technologies</h4>
      <p>
        Python • Pandas • NumPy • Pytest<br />
        GitHub Actions • Backtesting Engine<br />
        Performance Analytics
      </p>
      <h4>Validated One Box Strategy Example</h4>
      <ul>
        <li><strong>Trading days evaluated:</strong> 44</li>
        <li><strong>Trades generated:</strong> 11</li>
        <li><strong>Win rate:</strong> 63.64%</li>
        <li><strong>Profit factor:</strong> 3.50</li>
        <li><strong>Total return multiple:</strong> 10.00R</li>
      </ul>
      <h4>Strategy Research</h4>
      <p>
        One Box Breakout • MTF Confirmation<br />
        Risk-to-Reward Models • Intraday Trading<br />
        Historical Validation • Quantitative Research
      </p>
    </td>
  </tr>
</table>

---

## 📊 GitHub Statistics

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=ShivamPandey07&amp;show_icons=true&amp;theme=tokyonight&amp;hide_border=true" alt="Shivam Pandey GitHub statistics" height="170" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=ShivamPandey07&amp;theme=tokyonight&amp;hide_border=true" alt="Shivam Pandey GitHub contribution streak" height="170" />
</p>

---

## 🤝 Connect With Me

<p align="center">
  <a href="https://linkedin.com/in/shivampandey32000" target="_blank" rel="noreferrer"><img src="https://www.readmecodegen.com/api/social-icon?name=linkedin&amp;size=96" alt="LinkedIn" title="LinkedIn" width="50" height="50" /></a>&nbsp;&nbsp;&nbsp;&nbsp;
  <a href="mailto:shivampandey32000@gmail.com"><img src="https://www.readmecodegen.com/api/social-icon?name=gmail&amp;size=96" alt="Email" title="Email" width="50" height="50" /></a>&nbsp;&nbsp;&nbsp;&nbsp;
  <a href="https://github.com/ShivamPandey07" target="_blank" rel="noreferrer"><img src="https://www.readmecodegen.com/api/social-icon?name=github&amp;size=96" alt="GitHub" title="GitHub" width="50" height="50" /></a>
</p>

---

<p align="center">
  ⭐ If you find my work useful, consider following my projects and repositories.
</p>
